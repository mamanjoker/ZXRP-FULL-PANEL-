...
(async () => {
  try {
    await db.read();
    db.data ||= { applications: [], settings: { prefix: '!' }, tickets: [], welcome: { enabled:false, channel:null, message:'' } };
    await db.write();
  } catch (e) {
...
bot.on('guildMemberAdd', async member => {
  try {
    ...
    console.error('guildMemberAdd err', e);
  }
});

// moderation & ticket command (prefix-based)
...
app.post('/applications/:id/decision', ensureAuth, async (req,res)=>{ const id = req.params.id; const { decision } = req.body; await db.read(); const item = db.data.applications.find(a=>a.id===id); if(!item) return res.status(404).send('Not found'); item.status = decision; item.decisionBy = req.user.username || req.user.id; item.decisionAt = new Date().toISOString(); await db.write(); try{ const chId = process.env.LOG_CHANNEL_ID; if (chId && bot.isReady()){ const ch = await bot.channels.fetch(chId).catch(()=>null); if (ch && ch.isTextBased()){ const embed = new EmbedBuilder().setTitle(`Application ${decision}`).addFields({ name:'Applicant', value: item.name || item.discordTag || 'N/A', inline:true }, { name:'Decision By', value: item.decisionBy, inline:true }).setDescription(item.about || 'No details').setTimestamp(new Date()); ch.send({ embeds: [embed] }).catch(()=>null); } } }catch(e){ console.error('decision log error', e); } res.redirect('/applications/'+id); });
